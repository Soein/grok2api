...
Use UID/GID 10001 for both signer and guard. The parent directories must be traversable, input files readable (0400), and `runtime` writable (0700) by that UID. The output is atomically replaced at mode 0600; mount its directory, not the individual file. Run the oneshot as its configured user and verify `/readyz` before enabling the timer.
